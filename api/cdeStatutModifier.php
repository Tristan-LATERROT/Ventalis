...
if($_SERVER['REQUEST_METHOD'] == 'PUT') {
    ...
    if(!empty($data->orderPublicId) && !empty($data->orderStatusCode)) {
        // réception des données
        // on filtre les caractères
        // fonction : updateOrderStatusById(PDO $pdo, string $newStatus, string $orderId)
        $newStatus = htmlspecialchars(strip_tags($data->orderStatusCode));
        $orderId = htmlspecialchars(strip_tags($data->orderPublicId));
        $updatedOrder = updateOrderStatusById($pdo, $newStatus, $orderId);
        ...
        if($updatedOrder) {
            ...
            echo json_encode(["message" => "commande modifiee $orderId avec le statut $newStatus"]);
        } else {
            // la création de commande a échoué
            // code réponse 503
            http_response_code(503);
            // Encodage du fichier de réponse
            echo json_encode(["message" => "erreur impossible de modifier la commande"]);
        }
    }
